 ...
 */
function truncate($string, $length=140, $uri=null, $urilen=null, $parens=true, $ellipsis='…', $hastags=1)
{
	...
	{
		$uris = array();
		foreach (find_urls($string, $tidy=false) as $key => $url)
		{
			$dummy = 'URL' . $key;
	...
	$maxplainlen = $length - uri_mb_strlen($append, $urilen);
	
	// See if truncation will happen
	if (uri_mb_strlen($string, $urilen) > $maxplainlen)
	{
 ...
 */
function uri_mb_strlen($string, $urilen)
{
	// Find all urls
	$urls = find_urls($string, $tidy=false);
 ...
 */
function find_urls($text, $tidy=true)
{
